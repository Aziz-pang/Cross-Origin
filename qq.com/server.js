...
var server = http.createServer(function (request, response) {
    ...
    if (path === '/') {
        ...
        // let referer = request.headers['referer'] 获取请求头
        response.setHeader('Access-Control-Allow-Origin', '*') //‘*’可以替换为域名
        response.write(fs.readFileSync('public/data.json'))
        response.end()
    } else if (path === '/dataJSONP.js') {
        if (request.headers['referer'].indexOf('http://hacker.com:9999') === 0) {
            ...
            const stringRe = string.replace('{{data}}', data).replace('{{callback}}',query.callback)
